...
const navItems: NavItem[] = [
  ...
  { href: '/attendance/scan-monitor',         label: 'متابعة المسح',         icon: 'list', adminOnly: true },
  { href: '/memorization',                    label: 'حفظ الطلبة',           icon: 'quran', teacherVisible: true },
  { href: '/teacher-attendance',              label: 'حضور المعلمين',        icon: 'teacher-check', adminOnly: true },
  ...
  { href: '/dashboard/registration-requests', label: 'طلبات التسجيل',        icon: 'user-check', adminOnly: true },
  { href: '/roles',                           label: 'الأدوار والصلاحيات',   icon: 'shield', adminOnly: true },
  { href: '/users',                           label: 'المستخدمون',           icon: 'settings-users', adminOnly: true },
...
export default function Sidebar({ role, fullName, isOpen = false, onClose }: SidebarProps) {
  ...
  const visibleItems = navItems.filter(item =>
    role === 'admin' || (!item.adminOnly) || item.teacherVisible  // custom roles = teacher-like
  )

  const generalHrefs = ['/dashboard', '/attendance', '/auto-attendance', '/barcode-attendance',
    '/memorization', '/teacher-attendance', '/groups', '/notifications', '/messages']
  const generalItems = visibleItems.filter(i => generalHrefs.includes(i.href))
  const adminItems = visibleItems.filter(i => !generalHrefs.includes(i.href))
